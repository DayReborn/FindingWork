...
# 2.4 基于dpdk的用户态协议栈的实现（暂停）

**本章的目标要讲清楚协议栈里面做什么？**

## 【引入】连接百度会发生什么？

当你在浏览器中输入 "www.baidu.com" 并按下回车时，整个过程涉及多个技术层面的交互。以下是详细的流程分解：

---

### **一、URL解析与HSTS检查**
1. **协议补充**  
   浏览器自动补全协议（如 `https://`），现代浏览器默认使用HTTPS。若手动输入 `http://`，可能会触发后续的301重定向到HTTPS。
   
2. **HSTS强制跳转**  
   检查浏览器本地的HSTS（HTTP Strict Transport Security）列表。若百度域名在列表中，浏览器直接发起HTTPS请求，跳过HTTP。

---

### **二、DNS解析**
1. **查询顺序**  
   - 浏览器缓存 → 操作系统缓存（如 `hosts` 文件） → 本地路由器缓存 → ISP的DNS服务器。
   - 若缓存未命中，进入递归查询流程。

2. **递归查询流程**  
   - **根域名服务器**：返回 `.com` 顶级域名服务器的地址。
   - **顶级域名服务器**：返回 `baidu.com` 权威域名服务器的地址。
   - **权威域名服务器**：返回 `www.baidu.com` 的IP地址，可能返回多个IP（负载均衡）。

3. **DNS优化**  
   - 可能返回CDN边缘节点IP（基于用户地理位置），加速内容分发。

---

### **三、TCP连接建立（三次握手）**
1. **SYN**  
   客户端发送SYN包（序列号随机值X）到服务器，进入 `SYN_SENT` 状态。

2. **SYN-ACK**  
   服务器返回SYN-ACK包（序列号Y，确认号X+1），进入 `SYN_RECEIVED` 状态。

3. **ACK**  
   客户端发送ACK包（确认号Y+1），双方进入 `ESTABLISHED` 状态，完成三次握手。

---

### **四、TLS/SSL握手（HTTPS加密）**
1. **Client Hello**  
   客户端发送支持的TLS版本、加密套件列表、随机数A。

2. **Server Hello**  
   服务器选择TLS版本和加密套件，返回随机数B、服务器证书（含公钥）、可能请求客户端证书。

3. **证书验证**  
   - 浏览器验证证书链（根证书→中间证书→百度证书），检查域名匹配和有效期。
   - 使用CA的公钥验证证书签名（如DigiCert、GlobalSign等）。

4. **密钥交换**  
   - 客户端生成随机数Pre-master Secret，用服务器公钥加密后发送。
   - 双方通过随机数A、B、Pre-master生成会话密钥（Session Key）。
...
1. **请求发送**  
   - 方法：`GET / HTTP/1.1`  
   - 头部：`Host: www.baidu.com`、`User-Agent`、`Accept-Encoding`（压缩支持）、`Cookie`（如BAIDUID）等。
   - 若启用HTTP/2，请求会被多路复用（Multiplexing）优化。

2. **服务器处理**  
   - **负载均衡**：请求可能被分发到最近的服务器集群（如百度自研BWS系统）。
   - **后端逻辑**：生成动态内容（如搜索框、实时新闻）、读取缓存（如首页静态化）。

3. **响应返回**  
   - 状态码：`200 OK` 或 `304 Not Modified`（缓存生效）。
   - 头部：`Content-Type: text/html`、`Cache-Control`、`Set-Cookie`（会话管理）。
   - 数据：HTML文档分块传输（`Transfer-Encoding: chunked`）或通过HTTP/2流式传输。

---

### **六、浏览器渲染**
1. **解析HTML构建DOM**  
   - 边下载边解析，遇到 `<script>` 可能阻塞（除非标记 `async/defer`）。
   - 预加载扫描器（Preload Scanner）提前请求CSS、图片等资源。

2. **解析CSS生成CSSOM**  
   - 解析外部CSS文件，合并为CSSOM（层叠样式表对象模型）。

3. **构建渲染树（Render Tree）**  
   - 合并DOM和CSSOM，排除不可见元素（如 `<head>`、`display: none`）。

4. **布局（Layout/Reflow）**  
   - 计算每个节点的几何信息（位置、尺寸）。

5. **绘制（Painting）与合成（Compositing）**  
   - 将元素分层绘制到屏幕上，GPU加速合成（如CSS动画）。

6. **加载后续资源**  
   - 执行JavaScript（如统计代码、A/B测试逻辑）。
   - 加载图片、字体等，可能触发重新布局（如未指定图片尺寸）。

---

### **七、连接维护与关闭**
1. **Keep-Alive**  
   - 默认启用HTTP Keep-Alive，TCP连接复用后续请求（如页面内资源）。

2. **连接关闭（四次挥手）**  
   - 客户端发送FIN → 服务器ACK → 服务器发送FIN → 客户端ACK，等待TIME_WAIT状态（防止延迟报文干扰）。

---

### **八、附加技术点**
- **CDN加速**：静态资源（如图片、JS）可能从CDN节点就近获取。
- **缓存策略**：浏览器缓存HTML、CSS，后续访问可能直接读取（校验 `ETag` 或 `Last-Modified`）。
- **Cookie管理**：请求中携带BAIDUID等Cookie用于追踪会话，响应可能更新Cookie。

---

整个过程在数百毫秒内完成，涉及数十个网络节点和复杂的软件协作，展示了现代互联网基础设施的高度协同。

![image-20250410235532151](进阶学习/image-20250410235532151.png)

## 2.4.1 用户协议栈之tcpip设计

![image-20250411000001159](进阶学习/image-20250411000001159.png)

参考一下七层模型：

![image-20250411001749574](进阶学习/image-20250411001749574.png)

> **接收的数据如下：**
>
> 【以太网】【IP头】【TCP协议头】key value
...
![image-20250411011451550](进阶学习/image-20250411011451550.png)
...
RDMA（**远程直接内存访问**）是一种网络技术，允许计算机**绕过CPU和操作系统**，直接从一台机器的内存读取或写入另一台机器的内存，**大幅降低通信延迟和CPU开销**，适用于高性能计算、数据中心和存储场景。
...
1. **零拷贝（Zero-Copy）**
   数据直接从发送端内存传输到接收端内存，无需经过CPU、内核或应用层缓存。
2. **内核旁路（Kernel Bypass）**
   应用程序通过**用户态驱动**直接操作网卡，跳过操作系统协议栈（如TCP/IP）。
3. **硬件卸载（Offload）**
   RDMA网卡（RNIC）直接处理数据搬运、校验和等任务，减轻CPU负担。

------

#### **关键优势**

- **低延迟**：延迟可降至微秒级（传统TCP/IP为毫秒级）。
- **高吞吐**：支持100Gbps+带宽，适合大规模数据传输。
- **低CPU占用**：通信过程几乎不消耗CPU资源。



### 3. eBPF（扩展伯克利包过滤器）

eBPF（**Extended Berkeley Packet Filter**）是Linux内核的**可编程技术**，允许用户在不修改内核源码的情况下，向内核注入自定义程序，用于**网络、监控、安全、跟踪**等场景，实现灵活的内核级数据处理。

------

#### **核心原理**
1. **沙盒机制**  
   eBPF程序运行在内核的虚拟机中，通过严格验证确保安全性（如禁止循环、内存越界），避免内核崩溃。
2. **事件驱动**  
   程序可挂载到内核事件（如系统调用、网络数据包到达），实时响应并处理数据。
3. **零开销观测**  
   直接在内核态完成数据聚合或过滤，避免向用户态传递冗余信息。

#### **关键优势**
- **动态性**：无需重启系统即可动态加载/更新程序。
- **高性能**：内核态执行，减少用户态-内核态切换。
- **跨场景**：支持网络优化、性能分析、安全策略等多个领域。
...
XDP是Linux内核的**高性能网络数据平面框架**，基于eBPF实现，允许在**网络驱动层**（最早可能的数据处理阶段）执行自定义程序，适用于DDoS防御、负载均衡等低延迟场景。

------

#### **核心原理**
1. **早期处理位置**  
   数据包在网卡接收后**立即处理**（早于内核协议栈和内存分配），延迟极低。
2. **eBPF程序挂载**  
   用户编写的eBPF程序直接操作原始数据包，决定转发、丢弃或修改。
3. **三种模式**  
   - **原生模式**：在网卡驱动中运行（性能最佳）。
   - **卸载模式**：由智能网卡硬件执行（如DPU）。
   - **通用模式**：内核兼容层处理（兼容性高）。
...
#### **关键优势**
- **纳秒级延迟**：处理速度比传统iptables快10倍以上。
- **资源节省**：丢弃恶意流量时，无需分配内核内存。
- **灵活编程**：支持复杂逻辑（如自定义负载均衡算法）。

------

#### **协同关系**
- **eBPF是基础技术**，XDP是其**网络优化场景**的延伸。
- **XDP + RDMA**：在超低延迟网络中，XDP可快速预处理数据包，RDMA实现零拷贝传输，两者结合用于金融交易、AI训练等极致性能场景。
...
> 要不要硬件的支持？  一开始没必要
>
> testpmd, example 这些源码要不要一开始就读。  不用先把课上代码敲完实现起来
...
![image-20250422213815916](进阶学习/image-20250422213815916.png)
...
size=1g
runtime=30
filename=/home/wu/zzx/test_zzx  # 确保这是一个文件路径
group_reporting
create_on_open=1
directory=/home/wu/zzx          # 新增：指定工作目录，避免路径混淆
...
```bash
wu@wu-HP-Z640-Workstation:~/zzx/FindingWork/2.5.1_io_uring$ fio iops-test.fio 
random-read: (g=0): rw=randread, bs=(R) 4096B-4096B, (W) 4096B-4096B, (T) 4096B-4096B, ioengine=posixaio, iodepth=1
...
random-write: (g=0): rw=randwrite, bs=(R) 4096B-4096B, (W) 4096B-4096B, (T) 4096B-4096B, ioengine=posixaio, iodepth=1
...
mixed-io: (g=0): rw=randrw, bs=(R) 4096B-4096B, (W) 4096B-4096B, (T) 4096B-4096B, ioengine=posixaio, iodepth=1
...
fio-3.1
Starting 12 processes
Jobs: 12 (f=12): [r(4),w(4),m(4)][100.0%][r=44.5MiB/s,w=31.1MiB/s][r=11.4k,w=7952 IOPS][eta 00m:00s]
random-read: (groupid=0, jobs=12): err= 0: pid=14694: Tue Apr 22 22:28:01 2025
   read: IOPS=12.6k, BW=49.1MiB/s (51.5MB/s)(1472MiB/30003msec)
    slat (nsec): min=1002, max=365639, avg=5722.72, stdev=2662.72
    clat (nsec): min=730, max=8786.5k, avg=501204.48, stdev=240429.98
     lat (usec): min=7, max=8793, avg=506.93, stdev=240.40
    clat percentiles (usec):
     |  1.00th=[   21],  5.00th=[  169], 10.00th=[  241], 20.00th=[  326],
     | 30.00th=[  379], 40.00th=[  429], 50.00th=[  478], 60.00th=[  529],
     | 70.00th=[  586], 80.00th=[  652], 90.00th=[  775], 95.00th=[  930],
     | 99.00th=[ 1254], 99.50th=[ 1352], 99.90th=[ 1565], 99.95th=[ 1680],
     | 99.99th=[ 4359]
   bw (  KiB/s): min= 4496, max=11208, per=12.51%, avg=6285.48, stdev=1357.78, samples=479
   iops        : min= 1124, max= 2808, avg=1571.43, stdev=339.73, samples=479
  write: IOPS=8507, BW=33.2MiB/s (34.8MB/s)(997MiB/30003msec)
    slat (nsec): min=1241, max=334540, avg=5866.71, stdev=2848.48
    clat (usec): min=40, max=8896, avg=649.03, stdev=270.54
     lat (usec): min=42, max=8902, avg=654.90, stdev=270.74
    clat percentiles (usec):
     |  1.00th=[   58],  5.00th=[   91], 10.00th=[  408], 20.00th=[  490],
     | 30.00th=[  537], 40.00th=[  586], 50.00th=[  627], 60.00th=[  676],
     | 70.00th=[  734], 80.00th=[  807], 90.00th=[  955], 95.00th=[ 1139],
     | 99.00th=[ 1434], 99.50th=[ 1516], 99.90th=[ 1729], 99.95th=[ 1909],
     | 99.99th=[ 4817]
   bw (  KiB/s): min= 1763, max= 9090, per=12.51%, avg=4257.43, stdev=2077.45, samples=478
   iops        : min=  440, max= 2272, avg=1064.30, stdev=519.34, samples=478
  lat (nsec)   : 750=0.01%
  lat (usec)   : 10=0.03%, 20=0.53%, 50=1.02%, 100=2.41%, 250=5.05%
  lat (usec)   : 500=32.47%, 750=40.67%, 1000=12.21%
  lat (msec)   : 2=5.57%, 4=0.02%, 10=0.01%
  cpu          : usr=1.97%, sys=1.57%, ctx=633351, majf=0, minf=1053
  IO depths    : 1=100.0%, 2=0.0%, 4=0.0%, 8=0.0%, 16=0.0%, 32=0.0%, >=64=0.0%
     submit    : 0=0.0%, 4=100.0%, 8=0.0%, 16=0.0%, 32=0.0%, 64=0.0%, >=64=0.0%
     complete  : 0=0.0%, 4=100.0%, 8=0.0%, 16=0.0%, 32=0.0%, 64=0.0%, >=64=0.0%
     issued rwt: total=376912,255261,0, short=0,0,0, dropped=0,0,0
     latency   : target=0, window=0, percentile=100.00%, depth=1

Run status group 0 (all jobs):
   READ: bw=49.1MiB/s (51.5MB/s), 49.1MiB/s-49.1MiB/s (51.5MB/s-51.5MB/s), io=1472MiB (1544MB), run=30003-30003msec
  WRITE: bw=33.2MiB/s (34.8MB/s), 33.2MiB/s-33.2MiB/s (34.8MB/s-34.8MB/s), io=997MiB (1046MB), run=30003-30003msec

Disk stats (read/write):
  sda: ios=142759/254338, merge=0/1729, ticks=25485/8567, in_queue=20, util=99.71%
...
|   测试类型   | 读取 IOPS | 写入 IOPS | 混合读写（70%读/30%写） |
| :----------: | :-------: | :-------: | :---------------------: |
|  **随机读**  |   12.6k   |     -     |            -            |
|  **随机写**  |     -     |   8.5k    |            -            |
| **混合读写** |   11.4k   |   7.9k    |    总计约 **19.3k**     |
...
- 随机读 IOPS = 12.6k
  ...
  对于 4KB 小块随机读操作，此数值在 SATA SSD 或中端 NVMe SSD 中属于中等偏下水平。

  - 延迟分析：
    - 平均读取延迟：**506.93 微秒**
    - 99.99% 的请求延迟 ≤ **4.36 毫秒**
    - 高延迟（部分请求超过 8 毫秒）表明磁盘可能存在性能瓶颈或后台负载。

**(2) 写入性能**

- 随机写 IOPS = 8.5k

  写入性能低于读取性能，符合大多数磁盘特性（尤其是 TLC/QLC SSD 因写入放大导致性能下降）。

  - 延迟分析：
    - 平均写入延迟：**654.90 微秒**
    - 99.99% 的请求延迟 ≤ **4.82 毫秒**
    - 写入延迟高于读取，可能与磁盘的写缓存策略或 NAND 闪存物理特性有关。

**(3) 混合读写性能**

- **总计 IOPS ≈ 19.3k (读 11.4k + 写 7.9k)**
  混合负载下性能低于纯读/纯写场景，说明磁盘在多任务混合负载下资源争用明显。
...
|      指标      |            观察值            |                             说明                             |
| :------------: | :--------------------------: | :----------------------------------------------------------: |
| **磁盘利用率** |          **99.71%**          |             磁盘已接近满负荷运行，成为系统瓶颈。             |
|  **I/O 深度**  |    始终为 1 (`iodepth=1`)    | 单线程队列深度不足，未能充分发挥磁盘并行性（高端 NVMe SSD 需更高深度）。 |
|  **延迟分布**  |  标准差较大（240-270微秒）   | 延迟波动明显，可能因磁盘调度算法、后台任务或硬件性能不均导致。 |
|  **带宽限制**  | 读取 51.5MB/s，写入 34.8MB/s | 远低于 NVMe SSD 的理论带宽（如 PCIe 4.0 SSD 可达 7000MB/s）。 |
